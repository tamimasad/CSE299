...
def generate_standard_bangla(input_text: str, model_choice: str):
    if model_choice == "m2m100" and m2m_model and m2m_tokenizer:
        ...
        generated_tokens = m2m_model.generate(
            **encoded_bn,
            ...
            max_length=64  # Reduced to prevent runaway text
        )
...
async def process_text(req: TextRequest):
    try:
        response_text = generate_standard_bangla(req.message, req.model)
...
async def process_voice(audio: UploadFile = File(...), model: str = Form("banglat5")):
    ...
    try:
        audio_segment = AudioSegment.from_file(
            temp_audio_path).set_frame_rate(16000).set_channels(1)
        samples = np.array(audio_segment.get_array_of_samples()
                           ).astype(np.float32) / 32768.0

        input_features = whisper_processor(
            samples, sampling_rate=16000, return_tensors="pt").input_features.to(device)
        predicted_ids = whisper_model.generate(
            input_features, forced_decoder_ids=whisper_processor.get_decoder_prompt_ids(language="bn", task="transcribe"))
...
async def save_session(data: SessionData):
    ...
    with open(filepath, "w", encoding="utf-8") as f:
        json.dump({"title": data.title, "messages": [
                  m.dict() for m in data.messages]}, f, ensure_ascii=False, indent=4)
