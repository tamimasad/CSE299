...
# --- 2. Load Fine-Tuned BanglaT5 Model ---
MODEL_PATH = "csebuetnlp/banglat5"

print(f"Loading Fine-Tuned BanglaT5 from {MODEL_PATH}...")
...
def generate_translations(texts, dialect_name, batch_size=BATCH_SIZE, desc="Translating"):
    """
    Translate a list of texts in batches using the dialect-aware prefix
    that was used during fine-tuning.
    """
    all_predictions = []

    # Must match the prefix format used in train_BanglaT5.py
    prefixed_texts = [str(text) for text in texts]

    with tqdm(total=len(prefixed_texts), desc=desc, unit="lines") as pbar:
        for i in range(0, len(prefixed_texts), batch_size):
            ...
            with torch.no_grad():
                generated_tokens = model.generate(
                    **inputs,
                    num_beams=4,              # matches generation_num_beams in training
                    max_length=128,           # matches generation_max_length in training
                )
...
if __name__ == "__main__":
    ...
    print("\n" + "="*40)
    print("Original BanglaT5 PERFORMANCE:")
    print("="*40)
    print(results_df.to_string(index=False))

    results_df.to_csv("banglat5_benchmark_results.csv", index=False)
    print("\nResults saved to banglat5_benchmark_results.csv")
